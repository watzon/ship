...
Use a version that publishes release assets for your host platforms before relying on `ship agent upgrade` from CI — verify with `ship release check v0.5.1` before pinning. For runners or hosts without GitHub access, see [docs/airgap.md](airgap.md).
