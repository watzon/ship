...
`ship health ENV [SERVICE]` runs the configured command or HTTP health checks against the current release without deploying. Use `--replica N` to check one placed service replica and `--json` for monitors or incident tooling. `ship health`, `ship logs`, `ship restart`, and `ship exec` all accept SERVICE as a bare service name from `ship.yml`, the `service.N` shorthand shown in `ship ps` output (e.g. `web.2`), or a full container name copied from `ship ps`.
