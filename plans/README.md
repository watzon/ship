...
| [010 — Bounded host fan-out](010-bounded-host-fanout.md) | P3 | Performance | L | MED | BLOCKED | 002, 004, 005, 009 |
...
Plan 010 is blocked by its hidden-ordering STOP condition: current `provision apply` returns on the first host bootstrap failure, while candidate branch `watzon/plan-010-host-fanout` would continue mutating later hosts. Preserving fail-fast remote effects is incompatible with concurrent bootstrap without an approved behavior change.
